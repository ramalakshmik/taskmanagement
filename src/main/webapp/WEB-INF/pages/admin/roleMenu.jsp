			...
			<c:forEach var="entry" items="${roleAuthMap}">
				...
				<tr>
					...
					<td>
						 ...
						 <%-- <form:select path="menuList" items="${menuMap}" multiple="true" />  --%>
					</td>
					<td>
						 <a
...
<form:form modelAttribute="roleDto" action="${editRoleAuth}" method="POST">
	<table style="width: 80%;" align="center">
		<tr style="background-color:#0c5978;color:white;">
			...
			<c:forEach var="role1" items="${roleDto.roleList}" varStatus="status">
				 <input type="hidden" name="roleList[${status.index}].id" value="${role1.id}" />
				<tr>
					<td>${role1.role}</td>
					 <td>
